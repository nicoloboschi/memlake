...
async fn query_fetches_only_probed_clusters_and_warms_the_cache() {
    ...
    let nprobe = 4;
    let depths = ArmDepths { vector: 100, text: 100, graph: 100, nprobe, graph_seed_min: 0.3, exact_rerank: true };
...
async fn a_tag_filtered_query_filters_before_reading_any_payload() {
    ...
    let depths = ArmDepths { vector: 10, text: 0, graph: 0, nprobe: 8, graph_seed_min: 0.3, exact_rerank: true };
...
async fn graph_seeds_are_covered_by_the_hydrated_winners() {
    ...
    let node = QueryNode::open(&ns, Tokenizer::default()).await.unwrap();
    let depths = ArmDepths { vector: 20, text: 0, graph: 20, nprobe: 8, graph_seed_min: 0.3, exact_rerank: true };
...
async fn the_updated_window_reaches_past_the_arm_depth() {
    ...
    let depths = ArmDepths { vector: 10, text: 0, graph: 0, nprobe: 64, graph_seed_min: 0.3, exact_rerank: true };
    let metrics = mlake_store::QueryMetrics::new();
